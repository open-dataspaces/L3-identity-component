 ...
 *
 * This class is a DTO for API responses.
 *
 * Date: 2025/08/04
 */
...
/**
 * APIResponse is a generic class that represents a standard response structure for API calls.
 * It includes fields for type, title, status, detail, and data.
 *
...
public class APIResponse<T> {
    ...
    /**
     * The data contained in the response, which can be of any type.
     */
    @NonNull
    @JsonProperty(Const.JSON_PROPERTY_COMMON_DATA)
    private T data;

    /**
     * Constructs a new APIResponse with the specified status and data.
     *
     * @param status the HTTP success status code
     * @param data   the data to be included in the response
     */
    public APIResponse(HttpServletRequest request, int status, T data) {
        ...
        this.title = Const.API_RESPONSE_TITLE_SUCCESS;
        this.status = status;
